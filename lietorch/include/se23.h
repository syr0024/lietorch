...
class SE23 {
  public:
    const static int constexpr k = 2;
    ...
    EIGEN_DEVICE_FUNC SE23() { translation = Vector3k::Zero(); }

    EIGEN_DEVICE_FUNC SE23(SO3<Scalar> const& so3, Vector3k const& t) : so3(so3), translation(t) {};

    EIGEN_DEVICE_FUNC SE23(const Scalar *data) : so3(data), translation(data+4) {};

    EIGEN_DEVICE_FUNC SE23<Scalar> inv() {
      Vector3k trans_inv;
      for (int i=0; i<k; i++) {
        trans_inv.template segment<3>(3*i) = -(so3.inv()*translation.template segment<3>(3*i));
      }
      return SE23(so3.inv(), trans_inv);
    }
    ...
    EIGEN_DEVICE_FUNC static SE23<Scalar> Exp(Tangent const& phi_tau) {
      Vector3 phi = phi_tau.template segment<3>(0);
      Vector3k tau = phi_tau.template segment<3*k>(3);
    ...
    EIGEN_DEVICE_FUNC static Adjoint left_jacobian(Tangent const& phi_tau) {
      ...
      for (int i=0; i<k; i++) {
        ...
        Matrix3 Q = SE23<Scalar>::calcQ(tau_phi);
